...
namespace HashConstants {
    ...
    consteval std::array<std::array<std::array<uint64_t, 8>, 2>, 64> calcConstants();
}

#include <board/hash.impl>

namespace HashConstants {
    constexpr std::array<std::array<std::array<uint64_t, 8>, 2>, 64> CONSTANTS = calcConstants();
    constexpr uint64_t BLACK_STEP = nextRandomNumber(CONSTANTS[63][1][7]);
}
